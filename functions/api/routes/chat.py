...
from api.routes.drafts import run_quick_generate_for_tenant
...
def _topic_from_generate_post_message(text: str) -> str | None:
    low = text.strip()
    for pattern in (
        r"write a (?:linkedin )?post about\s+(.+)$",
        r"generate a post about\s+(.+)$",
        r"create a post about\s+(.+)$",
        r"draft a post about\s+(.+)$",
    ):
        m = re.search(pattern, low, re.I | re.DOTALL)
        if m:
...
async def _maybe_create_draft_from_chat_intent(
    intent: str | None,
    last_user_text: str,
    tenant: TenantProfile,
    tier: str,
) -> dict | None:
    if intent != "generate_post" or tier not in ("starter", "pro"):
        return None
    allowed_lim, _, _ = check_limit(
        tenant.tenant_id, tier, "post_generations_per_day", tenant.timezone
    )
    if not allowed_lim:
        return None
    try:
        raw_pf = list(tenant.platforms_enabled or []) or ["linkedin"]
        platforms = normalize_platforms(raw_pf)
        plat = platforms[0] if platforms else "linkedin"
        topic_guess = _topic_from_generate_post_message(last_user_text)
        draft_id, payload = await run_quick_generate_for_tenant(
            tenant, plat, topic_guess
        )
        increment_usage(
            tenant.tenant_id, "post_generations_per_day", tenant.timezone
        )
        headline = (payload.get("headline") or "")[:200]
        preview = ""
        cbp = payload.get("content_by_platform") or {}
        if isinstance(cbp, dict) and cbp:
            preview = (next(iter(cbp.values())) or "")[:280]
        return {"id": draft_id, "headline": headline, "preview": preview}
    except Exception as exc:
        logger.warning(
            "chat.draft_from_intent_failed",
...
async def chat(
    ...
    try:
        ...
        tier_chat = getattr(request.state, "tenant_tier", "starter")
        draft_created = await _maybe_create_draft_from_chat_intent(
            intent, last_user_text, tenant, tier_chat
        )
        if draft_created:
            reply = (
                f"{reply.rstrip()} I've added a draft to your queue — open Content Drafts "
                "to review and schedule it."
            )
...
async def chat_stream(
    ...
    async def event_gen():
        ...
        try:
            ...
            stream_tier = getattr(request.state, "tenant_tier", "starter")
            draft_created = await _maybe_create_draft_from_chat_intent(
                intent, last_message, tenant, stream_tier
            )
            if draft_created:
                reply = (
                    f"{reply.rstrip()} I've added a draft to your queue — open Content Drafts "
