...
class FirestoreSmokeStore:
    ...
    def update_tenant(self, tenant_id: str, updates: dict):
        existing = dict(self.tenants.get(tenant_id, {}))
        merged = {**existing, **updates}
        merged["id"] = tenant_id
        self.tenants[tenant_id] = merged
    ...
    def count_docs(self, collection: str, tenant_id: str | None = None):
        return len(self.query_docs(collection, tenant_id=tenant_id))
...
def test_publish_schedule_pipeline_smoke(monkeypatch):
    ...
    monkeypatch.setattr(drafts, "count_docs", store.count_docs)
    monkeypatch.setattr(drafts, "get_tenant", store.get_tenant)
    monkeypatch.setattr(drafts, "update_tenant", store.update_tenant)
    monkeypatch.setattr(
    ...
    assert analytics_payload["live_metrics_available"] is False
