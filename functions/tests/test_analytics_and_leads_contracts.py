...
def test_update_draft_status_creates_calendar_and_publishing_records(monkeypatch):
    ...
    monkeypatch.setattr(drafts, "count_docs", lambda *args, **kwargs: 0)
    monkeypatch.setattr(
        drafts,
        "get_tenant",
        lambda tid: {"tenant_id": tid, "approval_streak": 0},
    )
    monkeypatch.setattr(drafts, "update_tenant", lambda tid, up: None)
            ...
            return [
                {
                    "id": "2026-03-13",
                    "measured_at": "2026-03-13T00:00:00+00:00",
                    "metrics_source": "placeholder_until_platform_apis",
                    "post_metrics": [{"impressions": 0}],
                    "outreach_metrics": [],
                },
            ]
        if collection == "publishing_records":
            return []
        if collection in ("prospect_signals", "qualified_leads", "outreach_drafts"):
            return []
        return []
...
def test_linkedin_enrichment_uses_qualified_leads_collection(monkeypatch):
    ...
    assert update_calls[0][1]["enrichment_status"] == "skipped"
