...
async def publish_to_beehiiv(
    ...
    *,
    ...
    """Create a Beehiiv post via API v2. Returns (post_id, error_message)."""
    ...
    try:
        ...
        if resp.status_code in (200, 201):
            ...
            if isinstance(post, dict):
                ...
                if pid:
                    return (str(pid), None)
            return ("beehiiv_ok", None)
        body = resp.text[:300]
        logger.warning(
            "newsletter_providers.beehiiv_failed", extra={"status": resp.status_code}
        )
        return (None, f"Beehiiv API {resp.status_code}: {body}")
    except Exception as exc:
        logger.warning("newsletter_providers.beehiiv_error", extra={"error": str(exc)})
        return (None, str(exc))
