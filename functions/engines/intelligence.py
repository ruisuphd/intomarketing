...
from shared.competitor_intel import record_competitor_alert_if_needed
...
async def run_and_return_items(
    ...
    *,
    ...
    for s in surviving:
        ...
        try:
            add_doc("intelligence_items", dumped_item, tenant_id=tenant_id)
            record_competitor_alert_if_needed(tenant_id or "", dumped_item)
            stored_items.append(dumped_item)
        except Exception as exc:
            logger.error(
...
async def monitor_competitors(tenant_id: str):
    ...
    for comp in competitors:
        ...
        intel_dump = intel_item.model_dump()
        add_doc("intelligence_items", intel_dump, tenant_id=tenant_id)
        record_competitor_alert_if_needed(tenant_id, intel_dump)
