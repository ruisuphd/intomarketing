...
async def gather_daily_analytics() -> dict:
    """Iterate over tenants and store analytics snapshots.

    Post metrics come from LinkedIn/X APIs when tokens, org URN (LinkedIn), and
    external_ids exist; otherwise counts stay at zero with metrics_source
    ``placeholder_until_platform_apis``. Outreach email metrics remain zero until
    a provider supplies open/click data.
    """
