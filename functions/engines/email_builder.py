...
def _build_html(
    ...
    timezone_name: str,
    company_name: str = "",
    competitor_signal: dict | None = None,
) -> str:
    ...
    if competitor_signal:
        ct = competitor_signal.get("title") or "Competitor activity"
        src = competitor_signal.get("source_name") or ""
        summ = competitor_signal.get("summary") or ""
        competitor_html = f"""
        <div class="card" style="border-left:4px solid #e7a500">
          <h2>⚡ Competitor signal</h2>
          <p><strong>{_escape(src)}</strong> — {_escape(ct)}</p>
          {f'<p class="intel-summary">{_escape(summ)}</p>' if summ else ''}
        </div>"""
...
<body>
  <div class="card" style="background:#0073b1;color:#fff;padding:16px 24px;">
    <h1 style="color:#fff;margin:0">{_escape(company_name or 'IntoMarketing')} · Daily Brief</h1>
  ...
  {intel_html}
  {"".join(lead_sections)}
  <div class="footer">Generated by {_escape(company_name or 'IntoMarketing')} · {now_str} ({_escape(timezone_name)})</div>
</body>
</html>"""
