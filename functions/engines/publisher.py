    ...
    for record_data in due_records:
        ...
        try:
            ...
            if "id" in record_data:
                ...
                if REAL_PUBLISHING_ENABLED and provider in (
                    "linkedin",
                    "x",
                ) and not str(external_id).startswith("mock_"):
                    tdoc = get_tenant(tenant_id) or {}
                    if not tdoc.get("has_published_first_post"):
                        update_tenant(
                            tenant_id,
                            {
                                "has_published_first_post": True,
