...
def _get_competitor_signal(tenant_id: str) -> dict[str, Any] | None:
    """Return the top competitor intelligence item from the last 24 hours, or None."""
    now = datetime.now(timezone.utc)
    ...
    recent = [
        ...
        if (coerce_datetime(item.get("gathered_at")) or datetime.min.replace(tzinfo=timezone.utc)) >= day_ago
    ]
    if not recent:
        return None
    # Pick highest postability_score
...
async def dashboard_bootstrap(
    ...
    try:
        competitor_signal = _get_competitor_signal(tenant.tenant_id)
    except Exception:
