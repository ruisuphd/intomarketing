...
export default function ChatWidget() {
  ...
  const [suggestions, setSuggestions] = useState<string[]>([
    ...
    "Write a LinkedIn post about a product update",
              ...
              <div key={i} className={`flex flex-col ${msg.role === "user" ? "items-end" : "items-start"}`}>
                ...
                {msg.draftId && (
                  <Link
                    ...
                    className="mt-1.5 inline-flex text-xs font-medium text-apple-blue hover:underline"
                    onClick={() => window.dispatchEvent(new Event("drafts:changed"))}
                  >
                    Review draft in Content →
                  </Link>
                )}
                {msg.settingsUpdated && (
