    ...
    <section>
      ...
      {billing && !hasTierAccess(billing, "pro") ? (
        ...
        <div className="space-y-3">
          ...
          <p className="md:hidden text-xs text-apple-secondary mb-2">
            Stages stack vertically on small screens; use a larger screen for the full Kanban board.
          </p>
          <div className="flex flex-col gap-6 md:flex-row md:gap-4 md:overflow-x-auto md:pb-4 md:[touch-action:pan-x]">
            {COLUMNS.map((col) => {
              const columnLeads = leads.filter((l) => (l.status || "new") === col.id);
              return (
                <div
                  key={col.id}
                  className="flex w-full flex-col rounded-xl bg-apple-bg p-3 md:h-full md:min-h-[400px] md:w-80 md:shrink-0"
