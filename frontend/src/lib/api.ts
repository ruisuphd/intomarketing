...
export interface ChatDraftCreated {
  id: string;
  headline: string;
  preview: string;
}
...
export interface ChatStreamResult {
  ...
  draft_created?: ChatDraftCreated | null;
}
...
export async function apiChatStream(
  ...
  while (true) {
    ...
    for (const block of blocks) {
      ...
      if (obj.done === true) {
        const dc = obj.draft_created;
        let draft_created: ChatDraftCreated | null | undefined;
        if (dc && typeof dc === "object" && typeof (dc as { id?: unknown }).id === "string") {
          const o = dc as { id: string; headline?: string; preview?: string };
          draft_created = {
            id: o.id,
