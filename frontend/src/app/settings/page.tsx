...
export default function SettingsPage() {
  ...
  const [billingBusy, setBillingBusy] = useState<"portal" | "checkout" | null>(null);
  ...
  async function handleStripeCheckoutRedirect() {
    ...
    try {
      const data = await apiFetch<{ url?: string }>("/billing/checkout", {
        method: "POST",
        body: JSON.stringify({ tier: "pro", billing_interval: proBillingInterval }),
      });
      if (data.url) window.location.href = data.url;
    } catch (err: any) {
      setBillingMessage({
                        ...
                        <div className="space-y-3">
                          <div className="flex gap-2">
                            <button
                              ...
                              className={`flex-1 rounded-apple-sm border px-3 py-2 text-xs font-medium ${
                            ...
                            >
                              Annual (if configured)
                            </button>
                          </div>
                          <button
                            type="button"
                            onClick={() => void handleStripeCheckoutRedirect()}
                            disabled={billingBusy !== null}
                            className="w-full rounded-apple-sm border border-apple-border px-4 py-2.5 text-sm font-medium text-apple-text hover:bg-apple-bg disabled:opacity-50"
                          >
                            {billingBusy === "checkout" ? "Redirecting…" : "Checkout with Stripe (direct)"}
                          </button>
                          <button
                            type="button"
