...
export default function Onboarding() {
  ...
  const [firstPipelinePoll, setFirstPipelinePoll] = useState<"idle" | "waiting" | "ready" | "timeout">("idle");
    ...
    const tick = async () => {
      ...
      try {
        ...
        const done =
          lr?.status === "completed" ||
          lr?.status === "skipped" ||
          (typeof lr?.drafts_generated === "number" && lr.drafts_generated > 0);
        if (done) {
          setFirstPipelinePoll("ready");
          return;
        }
      } catch {
        /* keep polling */
      ...
      <div className="flex min-h-screen flex-col items-center bg-apple-bg px-4 py-20">
        <div className="w-full max-w-xl rounded-apple bg-apple-card p-10 shadow-apple text-center">
          ...
          <div className="mb-8 space-y-4 rounded-apple-sm border border-apple-border bg-apple-bg p-5 text-left">
            {firstPipelinePoll === "waiting" && (
              <p className="text-sm text-apple-text">
                <strong>Starting your first AI run…</strong> This usually takes 2–3 minutes. You can open the
                dashboard anytime; drafts will appear as soon as the run finishes.
              </p>
            )}
            {firstPipelinePoll === "ready" && (
              <p className="text-sm text-green-800">
                <strong>First run complete.</strong> Check your dashboard for new drafts and signals.
              </p>
            )}
            {firstPipelinePoll === "timeout" && (
              <p className="text-sm text-amber-800">
                Your first pipeline is still running or will finish shortly. Open the dashboard to see updates.
              </p>
            )}
            <p className="text-sm text-apple-text">
